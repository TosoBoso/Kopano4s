...
#	for S in $SCRIPTS ; do echo "chk $(basename $S)" >> check_scripts.out && sudo docker run -it -v ~/repo/Kopano4s/SPK-PKG/scripts/container:/mnt -e SHELLCHECK_OPTS="-e SC1090 -e SC2039 -e SC2034 -e SC2004" --rm koalaman/shellcheck $(basename $S) >> check_scripts.out ; done
	...
	for S in $SCRIPTS ; do echo "chk $S" >> check_scripts.out && sudo docker run -it -v ~/repo/Kopano4s/SPK-PKG/scripts/addon:/mnt -e SHELLCHECK_OPTS="-e SC1090 -e SC1091 -e SC2039 -e SC2034 -e SC2004 -e SC2183 -e SC2086" --rm koalaman/shellcheck "$S" >> check_scripts.out ; done
else
