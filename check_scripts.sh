...
then
	...
	for S in $SCRIPTS ; do echo "chk $S" >> check_scripts.out && sudo docker run -it -v ~/repo/Kopano4s/SPK-PKG/scripts:/mnt -e SHELLCHECK_OPTS="-e SC1090 -e SC2039 -e SC2034 -e SC2004" --rm koalaman/shellcheck "$S" >> check_scripts.out ; done
	...
	SCRIPTS=$(ls -p ~/repo/Kopano4s/SPK-PKG/scripts/container/*.sh)
	for S in $SCRIPTS ; do echo "chk $S" >> check_scripts.out && sudo docker run -it -v ~/repo/Kopano4s/SPK-PKG/scripts/addon:/mnt -e SHELLCHECK_OPTS="-e SC1090 -e SC2039 -e SC2034 -e SC2004" --rm koalaman/shellcheck "$S" >> check_scripts.out ; done
else
